...
3. **Apply the SQL**, in order, via the Supabase **SQL Editor** (paste each file's contents and Run):
   ...
   32. `supabase/migrations/0034_knockout_tiebreak.sql` — extends the knockout pick-order tiebreak. The order was "fewest total_points, ties broken by reverse draft order"; it now goes **fewest points → worst goal difference → an admin-entered manual tiebreak → reverse draft order**. Adds `profiles.knockout_tiebreak` (lower picks first; 0 = unset), the derived `_manager_goal_difference()` (sum of GF−GA across a manager's `phase='group'` teams' `final` matches), the admin-only `set_knockout_tiebreak(user_id, rank)` + `knockout_tiebreak_standings()` RPCs (the admin panel surfaces points + GD during `knockout_realloc` so a genuine tie can be resolved with the managers' vote), and re-`create or replace`s `resolve_knockout_realloc()` with the new order snapshot (everything else byte-identical to `0031`). Mirrors `lib/knockoutView.reallocPickOrder`. Apply alongside the matching app code; no recalc needed.
