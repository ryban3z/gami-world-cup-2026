...
- **Allocation order — OPEN (two candidates, decide before the swap opens):** when two+ managers want the same free agent, the pick order is taken in turn by one of:
  1. **Reverse snake-draft order** — whoever drafted *last* picks first. Rewards the draft tail (the managers who got the worst of the original draft get first crack at surviving free agents). Fixed/known up front, independent of how the tournament has gone.
  2. **Reverse-standings priority** — whoever is *worst-placed on the leaderboard* picks first (NBA-lottery style). A comeback mechanic: it helps whoever's actually behind, which may differ from draft order once results are in.

  Both are single-pass (one optional swap each) for v1. To be settled during play, before `knockout_realloc` opens.
