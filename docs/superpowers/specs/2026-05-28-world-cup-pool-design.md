...
- **Allocation mechanic — PLANNED (blind ranked preferences, auto-resolved). Implement later, not now.** Rather than a live, synchronous claim (hard across time zones — the same constraint that put the group draft on a 48h window), each manager privately submits, while `knockout_realloc` is open:
  ...
  The ranked-preference engine is order-agnostic; with the order now resolved (reverse standings, reverse-snake tiebreak — see above), managers are walked worst-placed-first. Sub-questions for the implementation plan: whether a second serpentine pass is ever allowed; how an empty/partial wishlist is treated (default: no swap); and confirming the dropped team is always named explicitly (not inferred). Likely repurposes the dormant `swap_nominations` table for the drop + ranked-claim state.
