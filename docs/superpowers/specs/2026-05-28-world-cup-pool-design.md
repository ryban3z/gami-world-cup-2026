...
- **Allocation order — RESOLVED 2026-06-10: reverse-standings priority, tiebroken by reverse snake-draft order.** When two+ managers want the same free agent, whoever is *worst-placed on the leaderboard* picks first (NBA-lottery style); managers level on points are ordered by reverse draft order (later original draft slot → earlier pickup). Single-pass (one optional swap each) for v1.
  - **Standings snapshot:** taken from `scores.total_points` **at resolve time**, so it reflects the **final end-of-group-stage standings**. (The window is opened a few days *earlier* — the gap between the last group game and the first R32 game is under a day, too short to fit the whole submit window — so the order can't be snapshotted at open; it's computed when the admin resolves, after a results refresh.)
  - *Rationale:* managers have no agency after the draft (points accrue automatically; bonus picks locked at kickoff), so the bottom of the table at the swap reflects variance more than skill — and tanking is impossible, which removes the classic objection to worst-picks-first. It also keeps trailing managers engaged. Reverse snake alone was rejected: the snake's back-to-back turn already compensated the draft tail, and no one drafted with a tail-priority swap expectation. (Reverse snake-draft order survives as the tiebreak.)
- **Allocation mechanic — BUILT (blind ranked preferences, auto-resolved; migration `0030`).** Rather than a live, synchronous claim (hard across time zones — the same constraint that put the group draft on a 48h window), each manager privately submits, while `knockout_realloc` is open:
  - the **one team they'll drop** from their roster, and
  - a **ranked wishlist** of **undrafted teams** they'd want, best-first. (Because the window opens during the group stage — before the R32 field exists — managers rank from the full pool of undrafted teams, flagged with their live qualified/eliminated status; only picks that actually reach R32 are eligible at resolve.)

  Submissions stay **blind** (consistent with the app's visibility model) and **editable** until the window closes. The system then resolves **automatically** in the chosen pick order (see the order question above): walk the managers in order; award each their highest-ranked team that **reached R32** and is still unclaimed; mark it taken; continue. A manager whose whole wishlist is already claimed (or never reached R32) keeps their roster unchanged, and **the drop only executes if a claim succeeds** (no claim → no change). One pass, one swap each; the result is revealed once allocation completes.

  The ranked-preference engine is order-agnostic; with the order now resolved (reverse standings, reverse-snake tiebreak — see above), managers are walked worst-placed-first. **Sub-questions resolved at build (migration `0030`):** single pass only (no second serpentine pass); the wishlist is capped at the **top 3**; an empty/partial wishlist (or a wishlist whose picks are all taken) is **no swap**; the dropped team is always **explicit** (never inferred). It repurposes `swap_nominations` for the drop + ranked-claim state (one row per manager per rank) and resolves via `resolve_knockout_realloc()`, which **snapshots the pick order from the final standings into `game_config.knockout_order`**, awards free agents, **materializes `phase='knockout'` ownership for every manager's final roster**, and is the only path to `knockout_locked`. Player UI: `app/(app)/knockout/`; the same window hosts the **wildcard** (`set_wildcard(category, pick_slot, value)` / `clear_wildcard()` — a **pending, editable** one-bonus-pick change held in `wildcard_choices` and applied at resolve, leaving the other slot of a two-answer category untouched).
...
> **Early-clinch qualify credit (added 2026-06-20, migration `0029`):** the qualify reward is now credited as soon as a team **mathematically clinches a top-2 group finish** — `deriveGroupQualified` in `lib/scoring.ts` reads each group's results and **enumerates every possible completion of the remaining group games** (3^n, n ≤ 6), marking a team qualified when in all of them at most one other team can finish at-or-above it (guaranteed 1st/2nd). Enumeration — not a per-rival "can they reach my points?" bound — is required, because two rivals that still have to play each other can't both win, so the simple bound under-credits real clinches (e.g. a leader on 6 pts whose two chasers face each other; this was the original USA/Group-D bug). This no longer waits for the R32 bracket to be populated. It's sound by construction (only marks teams that can't drop below 2nd, so it never over-awards); the **8 best-3rd-placed** qualifiers still can't be known until every group finishes, so they keep being credited later when their R32 fixture appears (the `furthest_stage ≥ r32` path). Surfaced as a `team_standings.qualified` flag and a green **"Qualified"** badge on the dashboard's "My teams" panel + roster cards. Recalc is idempotent, so applying mid-stage credits clinched teams retroactively.

> **Group points split (added 2026-06-20):** `ScoreBreakdown` now tracks `group_qualify` and `group_win` separately (with `group` kept as their sum). Shown as the Qualify/Wins cells on manager profile pages and the expandable leaderboard rows. The per-team `by_team` line stays merged (qualify + win on one row).
...
- [x] ~~Knockout re-allocation mechanic~~ — **Free-agent pickup** chosen (2026-06-03): one optional swap — drop one team + claim one unowned R32 team. Supersedes the earlier Option B blind swap; Option A (fresh snake draft) kept as fallback. **Allocation order — RESOLVED 2026-06-10: reverse-standings priority** (worst-placed on the leaderboard picks first), tiebroken by reverse snake-draft order; standings snapshotted from `scores.total_points` when the admin opens `knockout_realloc` (see Knockout Re-allocation). **Allocation mechanic — PLANNED, not yet built:** blind ranked preferences auto-resolved in pick order (each manager submits the team they'll drop + a ranked wishlist; see Knockout Re-allocation).
