...
- **Allocation order — RESOLVED 2026-06-10: reverse-standings priority, tiebroken by reverse snake-draft order.** When two+ managers want the same free agent, whoever is *worst-placed on the leaderboard* picks first (NBA-lottery style); managers level on points are ordered by reverse draft order (later original draft slot → earlier pickup). Single-pass (one optional swap each) for v1.
  - **Standings snapshot:** taken from `scores.total_points` at the moment the admin opens `knockout_realloc` (scores are rebuilt idempotently, so this is just a read). The snapshot is fixed for the whole window — a result or bonus resolution landing mid-window does not reshuffle the order.
...
- **Allocation mechanic — BUILT (blind ranked preferences, auto-resolved; migration `0030`).** Rather than a live, synchronous claim (hard across time zones — the same constraint that put the group draft on a 48h window), each manager privately submits, while `knockout_realloc` is open:
  ...
  - a **ranked wishlist** of unowned R32 teams they'd want, best-first.

  Submissions stay **blind** (consistent with the app's visibility model) until the window closes. The system then resolves **automatically** in the chosen pick order (see the order question above): walk the managers in order; award each their highest-ranked team that's still unclaimed; mark it taken; continue. A manager whose whole wishlist is already claimed keeps their roster unchanged, and **the drop only executes if a claim succeeds** (no claim → no change). One pass, one swap each; the result is revealed once allocation completes.

  The ranked-preference engine is order-agnostic; with the order now resolved (reverse standings, reverse-snake tiebreak — see above), managers are walked worst-placed-first. **Sub-questions resolved at build (migration `0030`):** single pass only (no second serpentine pass); the wishlist is capped at the **top 3**; an empty/partial wishlist (or a wishlist whose picks are all taken) is **no swap**; the dropped team is always **explicit** (never inferred). It repurposes `swap_nominations` for the drop + ranked-claim state (one row per manager per rank), snapshots the order into `game_config.knockout_order`, and resolves via `resolve_knockout_realloc()`, which **materializes `phase='knockout'` ownership for every manager's final roster** and is the only path to `knockout_locked`. Player UI: `app/(app)/knockout/`; the same window hosts the **wildcard** (`use_wildcard(category, pick_slot, value)` — one-time replacement of a single bonus pick, leaving the other slot of a two-answer category untouched).
